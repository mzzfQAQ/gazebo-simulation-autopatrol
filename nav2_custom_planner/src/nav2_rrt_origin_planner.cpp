...
#include "visualization_msgs/msg/marker.hpp" // 新增：用于 RVIZ 可视化
...
namespace nav2_rrt_origin_planner
{
    ...
    {
        RCLCPP_INFO(node_->get_logger(), "正在清理类型为 RRTOriginPlanner 的插件 %s", name_.c_str());
    }

    void RRTOriginPlanner::activate()
    {
        RCLCPP_INFO(node_->get_logger(), "正在激活类型为 RRTOriginPlanner 的插件 %s", name_.c_str());
    }

    void RRTOriginPlanner::deactivate()
    {
        RCLCPP_INFO(node_->get_logger(), "正在停用类型为 RRTOriginPlanner 的插件 %s", name_.c_str());
    }
        ...
        const geometry_msgs::msg::PoseStamped & goal)
    {
        ...
        for (int i = 0; i < max_iterations; ++i) {
            ...
            // 目标导向采样 (Goal Biasing) - 10% 的概率直接向目标生长，加速收敛
            if (i % 10 == 0) {
        ...
        RCLCPP_INFO(node_->get_logger(), 
            "RRT 规划结束 | 采样节点数: %ld | 耗时: %.2f ms | 结果: %s", 
